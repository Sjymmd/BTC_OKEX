...
for x in Coin[:int(Okex_Api._CoinLenth)]:
    try:
        ...
        ax2.legend(handles = [line2,line_], labels =['%s' % DataFrame.columns[3],'%s' % DataFrame.columns[4]])
        ...
        ax3.legend(line3, ('%s'%DataFrame.columns[5],))
        ...
        ax4.legend(line4, ('%s'%DataFrame.columns[6],))
